...
.INITR* \n[.F]
.SIZE 12 14
.TITLE CMPS-109 Spring\~2014 Program\~3 "Templates and Iterators"
.RCS \
...
The files
.V= *.tcc
are explicit template instantiations that should not be necessary,
since the compiler ought to be able to figure out these requirements
themselves and automatically perform the instantiations.
.V= \&.cpp
file
.E= after
the definition of the template function.
They are placed in separate files in order to keep the implementation
file separate from excessive coupling between implementation and client
modules.
.LE
